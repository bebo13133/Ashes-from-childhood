...
const NotificationDropdown = ({ notifications = [], onMarkAsRead, onMarkAllAsRead, onNotificationClick }) => {
    ...
    return (
        <div className='notification-dropdown-container' ref={dropdownRef}>
            <button className='notification-dropdown-btn' onClick={() => setIsOpen(!isOpen)} title='Нотификации'>
                <span className='notification-dropdown-icon'>🔔</span>
                {unreadCount > 0 && <span className='notification-dropdown-badge'>{unreadCount}</span>}
            </button>
                ...
                <div className='notification-dropdown-menu'>
                    <div className='notification-dropdown-header'>
                        <div className='notification-dropdown-title'>
                            <h3>Нотификации</h3>
                            {unreadCount > 0 && <span className='notification-dropdown-count'>{unreadCount} нови</span>}
                        </div>
                        {unreadCount > 0 && (
                            <button className='notification-dropdown-mark-all' onClick={handleMarkAllAsRead}>
                                Маркирай всички
                            </button>
                        )}
                    </div>
                    ...
                    <div className='notification-dropdown-list'>
                        {notifications.length === 0 ? (
                            ...
                            notifications.slice(0, 10).map((notification) => (
                                ...
                                >
                                    <div className='notification-dropdown-item-icon' style={{ backgroundColor: getNotificationColor(notification.type) }}>
                                        {getNotificationIcon(notification.type)}
                                    </div>

                                    <div className='notification-dropdown-item-content'>
                                        <div className='notification-dropdown-item-header'>
                                            <h4 className='notification-dropdown-item-title'>{getNotificationTitle(notification)}</h4>
                                            <span className='notification-dropdown-item-time'>
                                                {formatTime(notification.createdAt || notification.timestamp)}
                                            </span>
                                        </div>
                                        <p className='notification-dropdown-item-message'>{notification.message}</p>
                                    </div>

                                    {!notification.read && <div className='notification-dropdown-item-dot'></div>}
                                </div>
                            ))
                        )}
                    </div>
                    ...
                    {notifications.length > 10 && (
                        <div className='notification-dropdown-footer'>
                            <button className='notification-dropdown-view-all'>Виж всички нотификации</button>
                        </div>
                    )}
                </div>
            )}
